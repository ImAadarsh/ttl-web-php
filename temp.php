						...
						<div class="w-full p-60 text-center sm:w-full module-content" data-custom-animations="true" data-ca-options='{"addPerspective": false, "animationTarget": ".animation-element, .ld-fancy-heading", "startDelay" : 200, "initValues" :{"x": "-10px", "y": "10px", "opacity" : 0} , "animations" :{"x": "0px", "y": "0px", "opacity" : 1}}'>
							...
							<div class="lqd-contact-form lqd-contact-form-inputs-underlined lqd-contact-form-button-block lqd-contact-form-button-md lqd-contact-form-button-border-none lqd-contact-form-inputs-lg mb-30 animation-element">
								<div role="form" class="lqd-cf" lang="en-US">
									...
									<?php
// Assuming $attractions is available from your API response
$attractions = [
    [
        "attractionId" => 9,
        "attractionName" => "SKI DUBAI SNOW CLASSIC",
        "ticketTypeList" => [
            ["ticketTypeId" => 12, "ticketTypeName" => "SNOW CLASSIC", "apiTicket" => true]
        ]
    ],
    [
        "attractionId" => 12,
        ...
        "ticketTypeList" => [
            ["ticketTypeId" => 5, "ticketTypeName" => "WILDWADI STANDARD", "apiTicket" => false]
        ]
    ]
];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Handle form submission, process selected values, and generate the second dropdown options
    $selectedAttractionId = isset($_POST['attraction']) ? $_POST['attraction'] : null;

    // Find the selected attraction data
    $selectedAttraction = null;
    foreach ($attractions as $attraction) {
        if ($attraction['attractionId'] == $selectedAttractionId) {
            $selectedAttraction = $attraction;
            break;
        }
    }

    // Generate options for the second dropdown based on the selected attraction
    $secondDropdownOptions = '';
    if ($selectedAttraction) {
        foreach ($selectedAttraction['ticketTypeList'] as $ticketType) {
            if ($ticketType['apiTicket']) {
                $secondDropdownOptions .= '<option value="' . $ticketType['ticketTypeId'] . '">' . $ticketType['ticketTypeName'] . '</option>';
            }
        }
    }

    // Return the options as a response
    echo $secondDropdownOptions;
    exit;
}
?>

<!-- HTML Form -->
<form enctype="multipart/form-data" action="" method="POST" novalidate="novalidate" data-status="init">
    ...
    <div class="w-full relative flex">
        <span class="lqd-form-control w-full" data-name="ticket_type" style="text-align: start; ">
            <select name="ticket_type" class="lqd-form-control bg-transparent border-black text-black pl-30" aria-required="true" aria-invalid="false" placeholder="Choose Your Ticket Type">
                ...
                <!-- Options will be dynamically populated by the server based on the selected attraction -->
            </select>
        </span>
    </div>
